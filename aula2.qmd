...
url_base <- "http://www.camara.leg.br/sitcamaraws/SessoesReunioes.asmx/ListarDiscursosPlenario"
...
```{r cache=TRUE}
dados_sessoes <- xml2::read_xml(endereco) |>    # ler texto como XML
  xml2::xml_find_all("//sessoesDiscursos") |>   # elementos dentro de <sessoesDiscursos>
...
```{r}
kwic_defendeu_acusou <- corpus_sessoes |>
  tokens() |>
  kwic(c("defendeu", "acusou", "criticou", "repudiou")) |>
  head(15)

print(kwic_defendeu_acusou)
```

```{r}
kwic_nomes <- corpus_sessoes |>
  head(n = 100) |>
  tokens() |>
