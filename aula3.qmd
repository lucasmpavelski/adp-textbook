...
Análise de temas com dicionários:
...
meu_dicionario <- dictionary(list(
  ...
  educacao = c("educação", "escola", "escolas", "professor", "professores", "ensino", "universidade")
))

# Aplicar dicionário e criar DFM para contar as menções
dfm_temas <- corpus_discursos |>
  tokens(remove_punct = TRUE) |>
  tokens_tolower() |>
  tokens_lookup(dictionary = meu_dicionario) |>
  dfm()

# Contar a mensão de cada termo
...
Gerando n-gramas (sequências de palavras):
...
Skip-grams para capturar relações entre palavras não adjacentes:
...
# Gerar skip-grams (bi-gramas que podem pular até 2 palavras entre si) contendo a palavra "francisco"
...
Redes de co-ocorrência de palavras (FCM):
...
Identificando Collocations (expressões típicas):
...
Calculando TF-IDF para destacar termos distintivos:
...
Medindo a diversidade léxica (vocabulário):
...
Análise de leiturabilidade (complexidade do texto):
...
top_10_leiturabilidade <- leiturabilidade |>
  arrange(desc(Flesch)) |>
  head(10)
...
Identificando keyness entre grupos:
