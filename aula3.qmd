---
title: N-gramas, Collocations, TF-IDF e Análise de Tópicos
format: ipynb
---
...
```{r}
corpus_sessoes <- read_csv("data/dados_sessoes.csv") |>
  corpus()
...
collocations_sessoes <- corpus_sessoes |>
  textstat_collocations()

print(head(collocations_sessoes))
```

Podemos testar removendo stopwords:

```{r}
palavras_irrelevantes <- c(stopwords(language = "pt"), "n<c2><ba>")

collocations_sessoes <- corpus_sessoes |>
  tokens() |>
  tokens_tolower() |>
  tokens_remove(palavras_irrelevantes, padding = TRUE) |>
  textstat_collocations()

print((collocations_sessoes))
```
...
collocations_sessoes <- corpus_sessoes |>
  ...
  tokens_tolower() |>
  tokens_remove(palavras_irrelevantes, padding = TRUE) |>
  textstat_collocations(size = 2:4)
